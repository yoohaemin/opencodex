...
Pool/Direct использует `272000` по умолчанию, максимум `872000` и динамическое сжатие для каждого потока;
синхронизированный каталог показывает `max`, сохраняя при этом отдельную ступень `xhigh`.
